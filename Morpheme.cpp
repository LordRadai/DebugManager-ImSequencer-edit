...
int Morpheme::LoadTracks_Alt(sEventTrackData* track_base, MorphemeEventTrackList* track_list)
{
	if (!track_base)
		return 0;
...
/*
int Morpheme::LoadTracks(ImU64 track_base, MorphemeEventTrackList* track_list)
{
	uint64_t track_data;
	uint64_t* list;
	int trackCount = 0;
	int subcount_discrete = 0;
	int subcount_timed = 0;
	int index_subBlend = 0;
	int index_subGeneric = 0;

	//Blend
	if (track_base && *(int*)(track_base + 0x10) > 0)
	{
		trackCount = *(int*)((uint64_t)track_base + 0x10);

		list = (uint64_t*)*(uint64_t*)((uint64_t)track_base + 0x18);

		track_list->parent = (uint64_t)track_base;
		track_list->count_discrete = trackCount;
		track_list->tracks_discrete = new MorphemeEventTrack[trackCount];
		...
		for (int i = 0; i < trackCount; i++)
		{
			...
			track_list->tracks_discrete[i].parentId = -1;
			track_list->tracks_discrete[i].childId = -1;
			track_list->tracks_discrete[i].eventCount = *(int*)(track_data);
			track_list->tracks_discrete[i].eventId = *(int*)(track_data + 0x10);

			tmp = *(uint64_t*)(track_data + 0x8);
			track_list->tracks_discrete[i].trackName = (const char*)(tmp);

			tmp = *(uint64_t*)(track_data + 0x18);
			track_list->tracks_discrete[i].startTime = *(float*)(tmp);
			track_list->tracks_discrete[i].duration = *(float*)(tmp + 0x04);
			track_list->tracks_discrete[i].value = *(int*)(tmp + 0x08);
		}
		...
		{
			track_data = *(list + i);
			uint64_t tmp = *(uint64_t*)(track_data + 0x18);
			for (size_t j = 1; j < track_list->tracks_discrete[i].eventCount; j++)
			{
				if (index_subBlend < track_list->count_discreteSub)
				{
					track_list->tracks_discreteSub[index_subBlend].parentId = i;
					track_list->tracks_discreteSub[index_subBlend].childId = j;
					track_list->tracks_discreteSub[index_subBlend].eventCount = track_list->tracks_discrete[i].eventCount;
					track_list->tracks_discreteSub[index_subBlend].eventId = track_list->tracks_discrete[i].eventId;
					track_list->tracks_discreteSub[index_subBlend].trackName = track_list->tracks_discrete[i].trackName;

					track_list->tracks_discreteSub[index_subBlend].startTime = *(float*)(tmp + (byte)j * 0x0C);
					track_list->tracks_discreteSub[index_subBlend].duration = *(float*)(tmp + (byte)j * 0x0C + 0x04);
					track_list->tracks_discreteSub[index_subBlend].value = *(int*)(tmp + (byte)j * 0x0C + 0x08);
					//printf_s("Parent: %d\n", track_list->tracks_discreteSub[index_subBlend].parentId);
					//printf_s("BlendSub: [%d] (%f, %f, %d)\n", index_subBlend, track_list->tracks_discreteSub[index_subBlend].startTime, track_list->tracks_discreteSub[index_subBlend].duration, track_list->tracks_discreteSub[index_subBlend].value);
					index_subBlend++;
				}
			}
		}
	...
	//Generic
	if (track_base && *(int*)(track_base + 0x40) > 0)
	{
		trackCount = *(int*)((uint64_t)track_base + 0x40);

		list = (uint64_t*)*(uint64_t*)((uint64_t)track_base + 0x48);

		track_list->parent = (uint64_t)track_base;
		track_list->count_timed = trackCount;
		track_list->tracks_timed = new MorphemeEventTrack[trackCount];
		...
		for (int i = 0; i < trackCount; i++)
		{
			...
			track_list->tracks_timed[i].parentId = -1;
			track_list->tracks_timed[i].childId = -1;
			track_list->tracks_timed[i].eventCount = *(int*)(track_data);
			track_list->tracks_timed[i].eventId = *(int*)(track_data + 0x10);

			tmp = *(uint64_t*)(track_data + 0x8);
			track_list->tracks_timed[i].trackName = (const char*)(tmp);

			tmp = *(uint64_t*)(track_data + 0x18);

			track_list->tracks_timed[i].startTime = *(float*)(tmp);
			track_list->tracks_timed[i].duration = *(float*)(tmp + 0x04);
			track_list->tracks_timed[i].value = *(int*)(tmp + 0x08);		
		}
		...
		for (int i = 0; i < trackCount; i++)
		{
			...
			for (size_t j = 1; j < track_list->tracks_timed[i].eventCount; j++)
			{
				if (index_subGeneric < track_list->count_timedSub)
				{
					track_list->tracks_timedSub[index_subGeneric].parentId = i;
					track_list->tracks_timedSub[index_subGeneric].childId = j;
					track_list->tracks_timedSub[index_subGeneric].eventCount = track_list->tracks_timed[i].eventCount;
					track_list->tracks_timedSub[index_subGeneric].eventId = track_list->tracks_timed[i].eventId;
					track_list->tracks_timedSub[index_subGeneric].trackName = track_list->tracks_timed[i].trackName;

					track_list->tracks_timedSub[index_subGeneric].startTime = *(float*)(tmp + (byte)j * 0x0C);
					track_list->tracks_timedSub[index_subGeneric].duration = *(float*)(tmp + (byte)j * 0x0C + 0x04);
					track_list->tracks_timedSub[index_subGeneric].value = *(int*)(tmp + (byte)j * 0x0C + 0x08);
					//printf_s("Parent: %d\n", track_list->tracks_timedSub[index_subGeneric].parentId);
					//printf_s("GenericSub: [%d] (%f, %f, %d)\n", index_subGeneric, track_list->tracks_timedSub[index_subGeneric].startTime, track_list->tracks_timedSub[index_subGeneric].duration, track_list->tracks_timedSub[index_subGeneric].value);
					index_subGeneric++;
				}
			}
		}
...
/*void Morpheme::Save(MorphemeEventTrackList* track_list)
{
	uint64_t track_base;
	uint64_t track_data;
	uint64_t* list;
	...
	if (track_list->parent)
	{
		...
		if (track_list->count_discrete > 0)
		{
			...
			for (int i = 0; i < track_list->count_discrete; i++)
			{
				...
				*(int*)(track_data) = track_list->tracks_discrete[i].eventCount;
				*(int*)(track_data + 0x10) = track_list->tracks_discrete[i].eventId;

				tmp = *(uint64_t*)(track_data + 0x18);
				*(float*)(tmp) = track_list->tracks_discrete[i].startTime;
				*(float*)(tmp + 0x04) = track_list->tracks_discrete[i].duration;
				*(int*)(tmp + 0x08) = track_list->tracks_discrete[i].value;

				for (size_t j = 1; j < track_list->tracks_discrete[i].eventCount; j++)
				{
					if (index_subBlend < track_list->count_discreteSub)
					{
						*(float*)(tmp + (byte)j * 0x0C) = track_list->tracks_discreteSub[index_subBlend].startTime;
						*(float*)(tmp + (byte)j * 0x0C + 0x04) = track_list->tracks_discreteSub[index_subBlend].duration;
						*(int*)(tmp + (byte)j * 0x0C + 0x08) = track_list->tracks_discreteSub[index_subBlend].value;
						index_subBlend++;
					}
				}
			}
		...
		if (track_list->count_timed > 0)
		{
			...
			for (int i = 0; i < track_list->count_timed; i++)
			{
				...
				*(int*)(track_data) = track_list->tracks_timed[i].eventCount;
				*(int*)(track_data + 0x10) = track_list->tracks_timed[i].eventId;

				tmp = *(uint64_t*)(track_data + 0x18);
				*(float*)(tmp) = track_list->tracks_timed[i].startTime;
				*(float*)(tmp + 0x04) = track_list->tracks_timed[i].duration;
				*(int*)(tmp + 0x08) = track_list->tracks_timed[i].value;

				for (size_t j = 1; j < track_list->tracks_timed[i].eventCount; j++)
				{
					if (index_subGeneric < track_list->count_timedSub)
					{
						*(float*)(tmp + (byte)j * 0x0C) = track_list->tracks_timedSub[index_subGeneric].startTime;
						*(float*)(tmp + (byte)j * 0x0C + 0x04) = track_list->tracks_timedSub[index_subGeneric].duration;
						*(int*)(tmp + (byte)j * 0x0C + 0x08) = track_list->tracks_timedSub[index_subGeneric].value;
						index_subGeneric++;
					}
				}
			}
...
void Morpheme::Save_Alt(MorphemeEventTrackList* track_list)
{
